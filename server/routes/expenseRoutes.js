...
router.post('/', upload.array('billImages', 10), async (req, res) => {
  try {
    const {
      employeeId,
      employeeName,
    ...
    if (req.files && req.files.length > 0) {
      try {
        ...
        for (const file of req.files) {
          ...
          billImageUrls.push(result.secure_url);
        }
        console.log(`✓ All ${billImageUrls.length} files uploaded successfully`);
      } catch (uploadError) {
